...
function CachedRequest (request) {
  ...
  _request.invalidateCache = function(){
    self.invalidateCache.apply(self, arguments);
  }
...
CachedRequest.prototype.invalidateCache = function (){
  var self = this
  ,   args = arguments
  ,   options = args[0]
  ,   key;
  ...
  if (fs.existsSync(this.cacheDirectory + key + ".json")) {
    fs.unlinkSync(this.cacheDirectory + key + ".json");
  }
  if (fs.existsSync(this.cacheDirectory + key)) {
    fs.unlinkSync(this.cacheDirectory + key);
  }
